...
class RTDSDataStore:
    ...
    def add_attack(self, attack_data):
        ...
        # Update threat types and top attackers
        self.threat_types[attack_data.get('type', 'UNKNOWN')] += 1
        self.top_attackers[attack_data.get('source', 'Unknown')] += 1
        
        # Update current threats (attacks in last 5 minutes)
        current_time = time.time()
        recent_attacks = [a for a in self.recent_attacks if current_time - a['timestamp'] < 300]
        self.stats['current_threats'] = len(recent_attacks)
...
def get_recent_attacks():
    ...
    recent_attacks = list(data_store.recent_attacks)[-20:]
    formatted_attacks = []
...
def get_packet_timeline():
    ...
    for entry in list(data_store.packet_timeline)[-30:]:  # Last 30 entries
...
def monitor_log_file(log_file_path='integrated_rtds_alerts.log'):
    """Monitor the RTDS log file for new entries and parse real data"""
...
def parse_log_entry(log_line):
    """Parse log entries from rtds_monitor.py and update dashboard data"""
    try:
        # Parse different types of log entries
        if "DDoS" in log_line or "DDOS" in log_line:
            attack_data = {
                'type': 'DDoS',
                'source': extract_ip_from_log(log_line),
                'severity': 'HIGH',
                'details': log_line
            }
            data_store.add_attack(attack_data)
            data_store.stats['total_attacks'] += 1
            socketio.emit('new_attack', [attack_data])
            
        elif "SYN flood" in log_line or "SYN Flood" in log_line:
            attack_data = {
                'type': 'SYN Flood',
                'source': extract_ip_from_log(log_line),
                'severity': 'HIGH',
        ...
        elif "ARP spoof" in log_line or "ARP Spoof" in log_line:
            attack_data = {
                'type': 'ARP Spoof',
                'source': extract_ip_from_log(log_line),
                'severity': 'MEDIUM',
        ...
        elif "MALWARE" in log_line:
            file_data = {
                'filename': extract_filename_from_log(log_line),
                'status': 'MALWARE',
                'detections': extract_detections_from_log(log_line),
                'action': 'QUARANTINED' if 'QUARANTINED' in log_line else 'DETECTED'
            }
            data_store.add_file_scan(file_data)
            socketio.emit('new_file_scan', file_data)
            
        elif "SUSPICIOUS" in log_line:
            file_data = {
                'filename': extract_filename_from_log(log_line),
                'status': 'SUSPICIOUS',
                'detections': extract_detections_from_log(log_line),
        ...
        elif "packets/sec" in log_line:
            packet_count = extract_packet_count_from_log(log_line)
            if packet_count:
                data_store.stats['total_packets'] += packet_count
                data_store.update_packet_stats(packet_count)
...
def extract_filename_from_log(log_line):
...
def extract_detections_from_log(log_line):
    """Extract detection count from log line"""
    import re
    match = re.search(r'detections?:\s*(\d+)', log_line, re.IGNORECASE)
    return int(match.group(1)) if match else 0

def extract_packet_count_from_log(log_line):
    """Extract packet count from log line"""
    import re
    match = re.search(r'(\d+)\s*packets?/sec', log_line, re.IGNORECASE)
...
def start_background_tasks():
    ...
    # Start periodic stats broadcasting (real data only)
    def broadcast_stats():
        while True:
    ...
    def periodic_device_scan():
        while True:
            data_store.scan_network_devices()
            socketio.emit('devices_update', list(data_store.network_devices.values()))
            time.sleep(30)  # Scan every 30 seconds
    
    device_scan_thread = threading.Thread(target=periodic_device_scan, daemon=True)
...
if __name__ == '__main__':
    ...
    print("⚠️  Note: Dashboard shows REAL data only")
    print("📡 To see network attacks: Run 'sudo python3 rtds_monitor.py --iface wlan0'")
    print("📁 To see file scans: Monitor a directory with VirusTotal API")
